...
- `action` **recommended**: (str) "boost", "stream" or "auto". See Appending B of the
  [value](value.md#payment-actions) spec for details.
...
- `reply_address` (str) The pubkey of the lightning node that can receive payments for the sender. The node given
  ...
  be interpreted as a
  "[lightning address](https://github.com/andrerfneves/lightning-address/tree/master?tab=readme-ov-file#tldr)".
...
- Rust-V4V: <https://github.com/Conshax/Rust-V4V/blob/main/src/boostagram.rs>
